"""Conjunctive hyperedges: do smaller, specific groups beat tabular? (5-fold CV)

Tests the idea that hub dilution can be avoided by defining hyperedges as
CONJUNCTIONS of attributes (surgeon x unit x diagnosis), which yields small,
specific groups instead of huge provider/unit hubs. Constructions (each + self
-loops), with per-fold-tuned HGNN, vs tabular:
  - S x U          : (primary surgeon, unit)
  - S x U x DM     : + diabetes status
  - S x U x DxSig  : + full comorbidity signature (most specific)

Prints group-size stats (to show hubs shrinking) and raw/enc vs tabular.
Fold-honest; writes nothing.  PYTHONPATH=. python analysis/cv_hypergraph_conj.py
"""
...
print("[conj] loading + assembling...", flush=True)
raw = load_raw()
...
print(f"[conj] cohort={N:,}  base={y.mean()*100:.2f}%", flush=True)
...
# per-encounter attributes
surg_of = _norm(merged.get("PrimarySurgID", pd.Series([""] * N))).tolist()
dm_of = merged.get("Diabetes Mellitus", pd.Series([""] * N)).astype(str).tolist()
comorb_present = [c for c in COMORB if c in merged.columns]
dxsig_of = merged[comorb_present].astype(str).agg("|".join, axis=1).tolist() if comorb_present else [""] * N
    ...
    for r in range(N):
        ...
        for u in units_of.get(r, []):
            ...
            if "dm" in spec:
                key.append(dm_of[r])
            if "dxsig" in spec:
                key.append(dxsig_of[r])
            d[tuple(key)].append(r)
...
print(f"[conj] tabular best AUROC mean {best_tab.mean():.3f}\n", flush=True)

for spec, tag in [(["surg", "unit"], "SxU"), (["surg", "unit", "dm"], "SxUxDM"),
                  (["surg", "unit", "dxsig"], "SxUxDxSig")]:
    ...
    print(f"[{tag:10s}] {len(groups):6d} edges (size med={int(np.median(sizes))} max={sizes.max()}) | "
          f"HGNN_raw {rau.mean():.3f}/{np.mean(rap):.3f}  HGNN+RF {eau.mean():.3f}/{np.mean(eap):.3f}  || "
