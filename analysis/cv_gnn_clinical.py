"""Clinical-concept-node heterograph: do diagnosis/procedure nodes help? (nested CV)

Addresses the critique that prior graphs used clinical information only as
encounter node FEATURES, never as STRUCTURE. Here the graph has FIVE node types:
encounter, provider, care-unit, DIAGNOSIS (one node per comorbidity), and
PROCEDURE (one node per CPT). Patients connect through shared diagnoses and
procedures, so message passing flows patient -> shared-condition -> other
patients. Encounter node features remain the enriched set (incl. the comorbidity
flags and CPT), so this tests whether concept STRUCTURE adds beyond the features.

Runs GraphSAGE / GAT / R-GCN / HGT with feature selection and TPE nested in each
of 5 CV folds, vs the enriched tree. MEDHG_SMOKE=1 for a tiny validation run.

    PYTHONPATH=. python analysis/cv_gnn_clinical.py
"""
import glob
import os
...
print(f"[clin] {'SMOKE ' if SMOKE else ''}loading...", flush=True)
cpt_map = load_cpt()
raw = load_raw()
...
# sequence features (for the enriched encounter representation)
...
a3u = raw.enc_unit_edges.copy()
a3u["UnitType"] = (a3u["DepartmentID"].astype(str).str.replace(r"\.0+$", "", regex=True).map(ud["g"]).fillna(a3u["UnitType"]))
a3u["InTime"] = pd.to_datetime(a3u["InTime"]); a3u["LogID"] = a3u["LogID"].astype(str)
seqs = a3u.sort_values(["LogID", "InTime"]).groupby("LogID", sort=False)["UnitType"].apply(list)
...
# ---- five-node-type graph: enc, provider, unit, diagnosis, CPT ------
...
o = N + len(P_ids)
uidx = {d: o + i for i, d in enumerate(U_ids)}
o += len(U_ids)
dxidx = {d: o + i for i, d in enumerate(DX_ids)}
o += len(DX_ids)
cidx = {c: o + i for i, c in enumerate(CPT_ids)}
TOT = o + len(CPT_ids)
...
# diagnosis edges: encounter -> comorbidity node when present
for dx in DX_ids:
    vals = merged[dx].astype(str)
    present = ~vals.str.strip().str.lower().isin(["no", "0", "nan", "none", ""])
    dn = dxidx[dx]
    for r in np.nonzero(present.values)[0]:
        src += [int(r), dn]; dst += [dn, int(r)]; et += [4, 5]
# procedure edges: encounter -> its CPT node
for r in range(N):
...
NREL, NNT = 9, 5
print(f"[clin] nodes={TOT:,} (enc={N:,} P={len(P_ids)} U={len(U_ids)} "
      f"DX={len(DX_ids)} CPT={len(CPT_ids)}) edges={g.num_edges():,}", flush=True)
    ...
    def __init__(self, arch, ind, hid, nl, dp):
        ...
        for i in range(nl):
            ...
            if arch == "sage":
                ...
                self.layers.append(HGTConv(di, do // 4, 4, NNT, NREL, dropout=dp))
...
def train_eval(arch, Xn, yt, trm, vam, hp, return_test=None):
    set_seed(SEED)
    net = Net(arch, Xn.shape[1], hp["hid"], hp["nl"], hp["dp"]).to(dev)
    ...
    for ep in range(MAX_EPOCHS):
        net.train(); logit = net(g, Xn)[:N]; loss = lf(logit[trm], yt[trm])
        opt.zero_grad(); loss.backward(); opt.step(); net.eval()
        with torch.no_grad():
            p = torch.softmax(net(g, Xn)[:N], -1)[:, 1].cpu().numpy()
    ...
    if return_test is not None:
        ...
        with torch.no_grad():
            p = torch.softmax(net(g, Xn)[:N], -1)[:, 1].cpu().numpy()
        return roc_auc_score(y[return_test], p[return_test]), average_precision_score(y[return_test], p[return_test])
    return best
...
def build_nodefeat(tr):
    ...
    return torch.tensor(Xn, device=dev), Xall
...
for fi, (tr_all, te) in enumerate(skf.split(np.zeros(N), y)):
    ...
    print(f"[clin] fold {fi+1}/{K}  tree(enriched) AUROC={tree['au'][-1]:.3f}", flush=True)
    for arch in ARCHS:
        try:
            def obj(trial):
                ...
                return train_eval(arch, Xn, yt, trm, vam, hp)
            study = optuna.create_study(direction="maximize", sampler=TPESampler(seed=SEED))
            study.optimize(obj, n_trials=N_TRIALS)
            bp = study.best_params
            au, ap = train_eval(arch, Xn, yt, trm, vam, dict(hid=bp["hid"], nl=bp["nl"], lr=bp["lr"], dp=bp["dp"]), return_test=te)
            results[arch]["au"].append(au); results[arch]["ap"].append(ap)
            print(f"[clin] fold {fi+1} {arch:5s} AUROC={au:.3f} AUPRC={ap:.3f} (val {study.best_value:.3f})", flush=True)
        except Exception as e:
            print(f"[clin] fold {fi+1} {arch:5s} FAILED: {type(e).__name__}: {e}", flush=True)

print(f"\n=== Clinical-concept-node nested-CV ({K}-fold, TPE {N_TRIALS}/fold) | base {y.mean()*100:.2f}% ===")
bt = np.array(tree["au"])
