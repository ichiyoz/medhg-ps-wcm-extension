"""CV check: does the TPE-TUNED GNN beat a (fair) tabular baseline?

Cross-validates the best configuration found by the full TPE hyperparameter
search on the fixed (full-batch) code -- the honest counterpart to that search's
single-split test AUROC of 0.700. Per identical fold:
  - gnn_raw   : tuned GNN softmax head
  - gnn_enc   : RF on tabular + tuned-GNN encounter embedding
...
BEST tabular baseline per fold. Reuses run.py's assembly; writes nothing.
    ...
    PYTHONPATH=. python analysis/cv_hpsearch_best.py
"""
...
# Best config from the fixed-code TPE search (artifacts_hpsearch/best_config.json).
# batch-norm off, smaller net (64/32/16, attn 16), lr 0.012. max_epochs raised to
# 300 (patience 25) so early stopping, not the cap, ends training.
GNN_CFG = replace(C.DEFAULTS_TRAIN,
                  learning_rate=0.012062569552564989,
                  l2_reg=5.796959749422979e-06,
                  dropout=0.20952501896643447,
                  batch_norm=False,
                  hidden_dim_1=64, hidden_dim_2=32, hidden_dim_3=16,
                  attn_dim=16,
                  max_epochs=300, early_stop_patience=25)

print(f"[cv] TPE-best GNN: lr={GNN_CFG.learning_rate:.4f} bn={GNN_CFG.batch_norm} "
      f"dims={GNN_CFG.hidden_dim_1}/{GNN_CFG.hidden_dim_2}/{GNN_CFG.hidden_dim_3} "
      f"attn={GNN_CFG.attn_dim} dropout={GNN_CFG.dropout:.2f}", flush=True)
