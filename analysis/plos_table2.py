...
sd_map = dict(zip(merged["LogID"], pd.to_datetime(merged["SurgeryDate"], errors="coerce")))
orders_raw["SurgeryStart"] = orders_raw["LogID"].map(sd_map)
mask_pre = orders_raw["OrderTime_dt"] < orders_raw["SurgeryStart"]
n_pre_total = int(mask_pre.sum())
orders_pre = orders_raw.loc[mask_pre].copy()
print(f"[plos] pre-surgery orders retained: {n_pre_total:,} of "
      f"{len(orders_raw):,} ({n_pre_total/len(orders_raw)*100:.1f}%)", flush=True)
per_case = orders_pre.groupby("LogID").size()
per_case_full = pd.Series(0, index=merged["LogID"])
per_case_full.update(per_case)
med_len = int(per_case_full.median())
n_any = int((per_case_full > 0).sum())
print(f"[plos] encounters with any pre-surgery orders: "
      f"{n_any}/{N} ({n_any/N*100:.1f}%); median pre-surgery seq len = "
      f"{med_len} (over ALL encounters), median for those with any = "
      f"{int(per_case.median()) if len(per_case) else 0}", flush=True)
...
print(f"[plos] cohort={N:,}  PLOS base={y_all.mean()*100:.2f}%  "
      f"pre-op vocab={len(vocab)}  median seq len (any-case)={int(np.median(lengths))}",
      flush=True)
...
# CV
# ---------------------------------------------------------------------------
MODELS = ["rf_clin", "rf_prov", "rf_gru_orders", "rf_gru_orders_prov", "rf_gnn_emb"]
...
np.savez(OUT_DIR / "plos_table2_oof.npz", y=y_all, **{m: oof[m] for m in MODELS})
...
for m in ["rf_prov", "rf_gru_orders", "rf_gru_orders_prov", "rf_gnn_emb"]:
    dau, dap = [], []
    for _, te in folds:
...
pd.DataFrame(rows).to_csv(OUT_DIR / "plos_table2_results.csv", index=False)
print(f"\n[plos] saved OOF -> {OUT_DIR/'plos_table2_oof.npz'}")
print(f"[plos] saved results -> {OUT_DIR/'plos_table2_results.csv'}")
