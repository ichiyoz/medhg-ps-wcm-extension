"""CPT conjunctive-hyperedge sweep (5-fold CV) -- includes surgeon x CPT x dx x unit.

Self-loads LogID->PrimaryCPT from the cohort export CSV (Cohort2020-2026.csv;
discovered programmatically, header or positional). Builds conjunctive
hyperedges that include the procedure code (CPT, 46 distinct), tuned HGNN per
fold, vs tabular. Fold-honest; writes nothing.

    PYTHONPATH=. python analysis/cv_hypergraph_cpt.py
"""
import glob
import os
...
SPECS = [
    ...
    (["cpt", "unit"], "CPT.U"),
    (["cpt", "dxsig"], "CPT.Dx"),
    (["surg", "cpt", "unit"], "S.CPT.U"),
    (["surg", "cpt", "dm"], "S.CPT.DM"),
    (["surg", "cpt", "unit", "dm"], "S.CPT.U.DM"),
    (["surg", "cpt", "unit", "dxsig"], "S.CPT.Dx.U"),   # the requested construction
]
    ...
    for d in [str(C.DATA_DIR), "/Users/yiyezhang/Dropbox/Surgery"]:
        for p in glob.glob(os.path.join(d, "*.csv")):
            ...
            if str(head.iloc[0, 0]).strip() == "LogID":
                ...
                return _map(pd.read_csv(p, header=None, names=COHORT_COLS, low_memory=False))
...
print("[cpt] loading CPT + assembling...", flush=True)
cpt_map = load_cpt()
print(f"[cpt] CPT for {len(cpt_map):,} LogIDs, {len(set(cpt_map.values()))} distinct codes", flush=True)
...
attr = {
    ...
    "cpt": [cpt_map.get(l, "") for l in merged["LogID"]],
    "dm": merged.get("Diabetes Mellitus", pd.Series([""] * N)).astype(str).tolist(),
    "dxsig": (merged[present].astype(str).agg("|".join, axis=1).tolist() if present else [""] * N),
}
n_cpt = sum(1 for v in attr["cpt"] if v)
print(f"[cpt] cohort={N:,}  base={y.mean()*100:.2f}%  CPT matched {n_cpt:,}/{N:,}", flush=True)
...
def build_groups(spec):
    d = defaultdict(list)
    for r in range(N):
        if "surg" in spec and (not attr["surg"][r] or attr["surg"][r] in ("nan", "None", "")):
            continue
        if "cpt" in spec and not attr["cpt"][r]:
            continue
...
print(f"[cpt] tabular best AUROC mean {best_tab.mean():.3f}\n", flush=True)
...
for spec, tag in SPECS:
    ...
    print(f"[{tag:11s}] {len(groups):6d} edges (med={int(np.median(sizes))} max={sizes.max()}) | "
          f"raw {rau.mean():.3f} enc {eau.mean():.3f}/{np.mean(eap):.3f} || "
