"""FINAL Table 2 (manuscript-ready) on the order-sequence + provider block.

Retires the unit-sequence GRU / A3 features and installs the ORDER-SEQUENCE
GRU as the deployable candidate, under the RF-everywhere calibrated pooled-OOF
protocol (matches analysis/table2_final_dca.py):
...
Reported rows (retired unit-based rows omitted; noted in the log):
  1) rf_clin              -- RF on Xtab                                       (baseline)
  2) rf_gru_orders        -- RF on [Xtab + order-GRU embedding]               (deployable)
  3) rf_gru_orders_prov   -- RF on [Xtab + order-GRU embedding + prov block]  (candidate)
  4) rf_prov              -- RF on [Xtab + prov block only]

  rf_gnn_emb (RF on Xtab + GNN encounter embedding) is skipped: build_graph()
  needs A3-unit features that are retired in this cohort. Noted in the log.

Also redoes the DCA on these five OOF panels (thresholds 0.02..0.30).

    PYTHONPATH=. python analysis/table2_orders_final.py
"""
...
# ---------------------------------------------------------------------------
print(f"[t2o] {'SMOKE ' if SMOKE else ''}loading...", flush=True)
...
print(f"[t2o] cohort={N:,}  base={y_all.mean()*100:.2f}%  "
      f"vocab={len(vocab)}  median seq len={int(np.median(lengths))}  "
      f"tab feats={len(feat_cols)}  prov feats={len(prov_cols)}", flush=True)
print("[t2o] SKIPPED (unit-sequence retired): rf_seq_units, rf_gru_units",
      flush=True)
print("[t2o] SKIPPED (needs A3-unit graph features): rf_gnn_emb", flush=True)
...
for fi, (tr_all, te) in enumerate(skf.split(np.zeros(N), y_all)):
    ...
    line = f"[t2o] fold {fi+1}/{K}  gru_val={vb:.3f} | "
    for m in MODELS:
        p = oof[m][te]
...
np.savez(OUT_DIR / "table2_orders_final_oof.npz", y=y_all,
         **{m: oof[m] for m in MODELS})

rows = []
print(f"\n=== FINAL Table 2 (calibrated pooled OOF; N={N:,}; base "
...
# also rf_gru_orders_prov vs rf_gru_orders (provider add-on)
dau, dap = [], []
for _, te in folds:
...
pd.DataFrame(rows).to_csv(OUT_DIR / "table2_orders_final_results.csv", index=False)
...
pd.DataFrame(dca_rows).to_csv(OUT_DIR / "table2_orders_final_dca.csv", index=False)
with open(OUT_DIR / "table2_orders_final_dca.json", "w") as f:
    json.dump(dict(N=N, prevalence=prev, nb_at=nb_at), f, indent=2)
...
try:
    ...
    ax.set_title(f"DCA — final Table 2 (N={N}, base {prev*100:.2f}%)")
    ax.set_xlim(0.02, 0.30)
    lo = min(curves[m].min() for m in MODELS) - 0.01
    ax.set_ylim(min(lo, treat_all.min() - 0.005), 0.06)
    ax.legend(loc="upper right", fontsize=8)
    plt.tight_layout()
    plt.savefig(OUT_DIR / "table2_orders_final_dca.png", dpi=180)
    print(f"[t2o] wrote {OUT_DIR/'table2_orders_final_dca.png'}")
except Exception as e:
    print(f"[t2o] DCA plot skipped: {e}")

print(f"\n[t2o] wrote:"
      f"\n  {OUT_DIR/'table2_orders_final_results.csv'}"
      f"\n  {OUT_DIR/'table2_orders_final_dca.csv'}"
      f"\n  {OUT_DIR/'table2_orders_final_dca.json'}"
      f"\n  {OUT_DIR/'table2_orders_final_oof.npz'}")
